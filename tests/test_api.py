    ...
    def predict(self, text: str) -> PredictionResult:
        return {
            ...
            "top_3": [
                ...
                {"category": "Hardware", "probability": 0.03},
                {"category": "Storage", "probability": 0.02},
            ],
        }
...
def test_routing_logic(client: TestClient) -> None:
    response = client.post(
        "/predict",
...
def test_top3_sorted(client: TestClient) -> None:
    response = client.post(
        "/predict",
...
def test_empty_ticket(client: TestClient) -> None:
